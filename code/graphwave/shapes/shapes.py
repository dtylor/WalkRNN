...
def barbel_graph(start, n1, n2, role_start=0, plot=False):
    ...
    graph.add_nodes_from(list(range(start, start + 2 * n2 + 1)))
...
def cycle(start, len_cycle, role_start=0, plot=False):
    ...
    '''
    graph = nx.Graph()
    graph.add_nodes_from(list(range(start, start + len_cycle)))
...
def diamond(start, role_start=0, plot=False):
    ...
    '''
    graph = nx.Graph()
    graph.add_nodes_from(list(range(start, start + 6)))
...
def house(start, role_start=0, plot=False):
    ...
    '''
    graph = nx.Graph()
    graph.add_nodes_from(list(range(start, start + 5)))
    graph.add_edges_from([(start, start + 1), (start + 1, start + 2),
                          (start + 2, start + 3), (start + 3, start)])
...
def star(start, nb_branches, role_start=0, plot=False):
    ...
    '''
    graph = nx.Graph()
    graph.add_nodes_from(list(range(start, start + nb_branches + 1)))
    for k in range(1, nb_branches + 1):
...
def string(start, width, role_start=0):
    ...
    '''
    graph = nx.Graph()
    graph.add_nodes_from(list(range(start, start + width)))
...
def tree(start, nb_levels, regularity, role_start=0, plot=False):
    ...
    graph.add_nodes_from(list(range(start, start + np.sum(nodes_level))))
    a = start
    it = 0
